...
#[verifier::ext_equal]
pub struct ParseMachineView {
    pub tasks: Seq<ParseTaskView>,
    pub completed: Option<ParsedNodeView>,
    pub fuel: u64,
}
...
impl View for ParseMachine {
    type V = ParseMachineView;

    closed spec fn view(&self) -> ParseMachineView {
        ParseMachineView {
            tasks: cst_parse_task_views_spec(self.tasks@),
            completed: cst_completed_node_view_spec(self.completed),
            fuel: self.fuel,
        }
    }
}
...
pub open spec fn cst_push_mapping_entries_from_spec(
    builder: CstBuilderView,
    tokens: Seq<crate::token::CompletedTokenView>,
    entries: Seq<CstMappingEntryView>,
    index: int,
    ...
    decreases fuel,
{
    if index < 0 || index > entries.len() || fuel == 0 || index >= entries.len() {
        ...
        match cst_push_mapping_entry_spec(builder, entry, offset) {
            Ok(next) => cst_push_mapping_entries_from_spec(
                next,
                tokens,
                entries,
                index + 1,
...
pub open spec fn cst_finish_iterative_mapping_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    closer: Option<u64>,
    builder: CstBuilderView,
    ...
    let entry_start = builder.mapping_entries.len() as u64;
    match cst_push_mapping_entries_from_spec(
        builder,
        tokens,
        task.pending_mapping,
        0,
        (task.pending_mapping.len() + 1) as nat,
    ) {
        Err(error) => Err(error),
        Ok(after_entries) => {
            let bounds = match closer {
            ...
            match bounds {
                ...
                Some((token_end, byte_end, next_token, offset)) => {
                    let node = CstNodeView {
                        ...
                        collection_end_token: closer,
                        entry_start,
                        entry_end: after_entries.mapping_entries.len() as u64,
                        empty_anchor_token: None,
                        empty_anchor_byte: None,
                    };
                    match cst_push_node_spec(after_entries, node, offset) {
...
fn push_iterative_mapping_entries(
    tokens: &[CompletedToken],
    entries: &[CstMappingEntry],
    builder: &mut CstBuilder,
) -> (result: Result<(), CstError>)
    ensures
        final(builder).syntax_owner_slots.len() == old(builder).syntax_owner_slots.len(),
        cst_push_mapping_entries_from_spec(
            old(builder)@,
            crate::token::completed_token_views_spec(tokens@),
            cst_mapping_entry_views_spec(entries@),
            0,
            (entries.len() + 1) as nat,
        ) == match result {
            Ok(()) => Ok(final(builder)@),
...
{
    let ghost expected = cst_push_mapping_entries_from_spec(
        builder@,
        crate::token::completed_token_views_spec(tokens@),
        cst_mapping_entry_views_spec(entries@),
        0,
    ...
    while pending_index < entries.len()
        invariant
            ...
            fuel == (entries.len() - pending_index + 1) as nat,
            builder.syntax_owner_slots.len() == old(builder).syntax_owner_slots.len(),
            expected == cst_push_mapping_entries_from_spec(
                old(builder)@,
                crate::token::completed_token_views_spec(tokens@),
                cst_mapping_entry_views_spec(entries@),
                0,
        ...
        decreases entries.len() - pending_index,
    {
        ...
        let ghost current_builder = builder@;
        proof {
            lemma_cst_mapping_entry_view_at(entries@, pending_index as int);
            assert(cst_mapping_entry_views_spec(entries@)[pending_index as int] == current@);
        }
        let offset = byte_at(tokens, current.token_start as usize, builder.source_len_bytes);
        proof {
            assert(offset == cst_byte_at_spec(
        ...
        match builder.push_mapping_entry(current, offset) {
            Ok(()) => {
                proof {
                    assert(cst_push_mapping_entry_spec(current_builder, current@, offset) == Ok(
                        builder@,
                    ));
                    reveal(cst_push_mapping_entries_from_spec);
                    fuel = (fuel - 1) as nat;
                }
            },
            Err(error) => {
                proof {
                    assert(cst_push_mapping_entry_spec(current_builder, current@, offset) == Err(
                        error@,
                    ));
                    reveal(cst_push_mapping_entries_from_spec);
                    assert(expected == Err(error@));
        ...
        },
{
    ...
    let ghost initial_builder = builder@;
    let ghost expected = cst_finish_iterative_mapping_spec(
        token_views,
        task_view,
        closer_view,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        assert(token_views.len() == tokens.len());
        assert(task_view.pending_mapping == cst_mapping_entry_views_spec(task.pending_mapping@));
        assert(task_view.pending_mapping.len() == task.pending_mapping.len());
    }
    let entry_start = builder.mapping_entries.len() as u64;
    match push_iterative_mapping_entries(tokens, &task.pending_mapping, builder) {
        Ok(()) => {
            proof {
                assert(cst_push_mapping_entries_from_spec(
                    initial_builder,
                    token_views,
                    task_view.pending_mapping,
                    0,
                    (task_view.pending_mapping.len() + 1) as nat,
                ) == Ok(builder@));
            }
        },
        Err(error) => {
            proof {
                ...
                reveal(cst_finish_iterative_mapping_spec);
                assert(expected == Err(error@));
            }
            return Err(error);
        },
...
pub open spec fn cst_resume_parse_task_spec(tasks: Seq<ParseTaskView>, task: ParseTaskView) -> Seq<
    ParseTaskView,
> {
    tasks.push(task)
}
...
fn resume_parse_task(tasks: &mut Vec<ParseTask>, task: ParseTask)
    ensures
        ...
        cst_parse_task_views_spec(final(tasks)@) == cst_resume_parse_task_spec(
...
pub open spec fn cst_initial_parse_machine_spec(
    start: u64,
    end: u64,
    allow_block_mapping: bool,
    depth_left: u64,
    ...
    ParseMachineView {
        tasks: cst_initial_parse_tasks_spec(start, end, allow_block_mapping, depth_left),
        completed: None,
        fuel: cst_initial_parse_fuel_spec(),
    }
}
...
fn initial_parse_machine(
    ...
    requires
        start <= end,
    ensures
        machine@ == cst_initial_parse_machine_spec(
            start as u64,
            end as u64,
            allow_block_mapping,
            depth_left,
        ...
        machine.tasks.len() == 1,
        cst_parse_task_stack_is_valid_spec(machine@.tasks),
{
    let tasks = initial_parse_tasks(start, end, allow_block_mapping, depth_left);
    let fuel = initial_parse_fuel();
    let machine = ParseMachine { tasks, completed: None, fuel };
    proof {
        reveal(cst_initial_parse_machine_spec);
    }
    machine
...
pub open spec fn cst_step_flow_sequence_state_zero_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
    ...
    if task.kind != ParseTaskKind::FlowSequence || task.state != 0 || task.end > tokens.len()
        ...
        if cursor >= task.end {
            ...
            if kind == CompletedTokenKind::FlowSequenceEnd {
            ...
            } else if kind == CompletedTokenKind::FlowEntry {
                Err(
                    CstErrorView {
                ...
                let keyed = if explicit {
                    let next = cst_skip_trivia_spec(
                        ...
                        cursor + 1,
                        task.end as int,
                        (task.end as int - cursor) as nat,
                    );
                    cst_task_set_cursor_spec(started, next as u64)
                } else {
                    started
                };
                if keyed.cursor >= task.end || tokens[keyed.cursor as int].kind
                    == CompletedTokenKind::MappingValue || tokens[keyed.cursor as int].kind
                    == CompletedTokenKind::FlowEntry || tokens[keyed.cursor as int].kind
                    == CompletedTokenKind::FlowSequenceEnd {
                    match cst_empty_node_spec(builder, tokens, keyed.cursor) {
                        Ok((next_builder, node_index)) => {
                            let with_key = cst_task_set_key_node_spec(keyed, node_index);
                    ...
                    match cst_machine_push_task_spec(
                        ...
                        child,
                        depth_limit,
                        tokens[keyed.cursor as int].byte_start,
                    ) {
...
pub open spec fn cst_step_flow_sequence_state_one_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::FlowSequence || task.state != 1 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
            ...
            Some((taken, child)) => {
                if child.next_token > task.end {
                    ...
                    let with_key = cst_task_set_key_node_spec(task, child.node_index);
                    let cursor = cst_skip_trivia_spec(
                        tokens,
                        child.next_token as int,
                        task.end as int,
                        (task.end - child.next_token) as nat + 1,
                    );
                    let positioned = cst_task_set_cursor_spec(with_key, cursor as u64);
                    let waiting = cst_task_set_state_spec(positioned, 2);
...
pub open spec fn cst_step_flow_sequence_finish_entry_spec(
    machine: ParseMachineView,
    builder: CstBuilderView,
    task: ParseTaskView,
    node_index: u64,
    ...
    let entry = CstSequenceEntryView {
        ...
        token_end: task.cursor,
        indicator_token: None,
    };
    let appended = cst_task_push_sequence_entry_spec(task, entry);
    let waiting = cst_task_set_state_spec(appended, 4);
    Ok((cst_machine_resume_task_spec(machine, waiting), builder))
}

pub open spec fn cst_step_flow_sequence_finish_pair_spec(
    machine: ParseMachineView,
    task: ParseTaskView,
    result: Result<(CstBuilderView, u64), CstErrorView>,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    match result {
        Ok((builder, pair)) => {
            cst_step_flow_sequence_finish_entry_spec(machine, builder, task, pair)
        },
        Err(error) => Err(error),
    }
}

pub open spec fn cst_step_flow_sequence_state_two_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
    ...
    if task.kind != ParseTaskKind::FlowSequence || task.state != 2 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else if task.cursor < task.end && tokens[task.cursor as int].kind
        == CompletedTokenKind::MappingValue {
        if task.entry_token_start >= task.end {
            Err(cst_step_node_internal_error_spec())
        } else if !task.explicit_key && tokens[task.entry_token_start as int].start_line_number
            != tokens[task.cursor as int].start_line_number {
            Err(
                CstErrorView {
                    kind: CstErrorKind::MultilineImplicitKey,
                    byte_offset: tokens[task.cursor as int].byte_start,
                },
            )
        } else {
            let with_colon = cst_task_set_colon_spec(task, task.cursor);
            let cursor = cst_skip_trivia_spec(
                tokens,
            ...
            let positioned = cst_task_set_cursor_spec(with_colon, cursor as u64);
            if cursor >= task.end || tokens[cursor].kind == CompletedTokenKind::FlowEntry
                || tokens[cursor].kind == CompletedTokenKind::FlowSequenceEnd {
                match cst_empty_node_spec(builder, tokens, cursor as u64) {
                    Err(error) => Err(error),
                    Ok((with_empty, value)) => {
                        let pair_end = if cursor > task.cursor {
                            cursor as u64
                        } else if task.cursor >= task.end {
                            task.cursor
                        } else {
                            (task.cursor + 1) as u64
                        };
                        if task.entry_token_start > pair_end || pair_end > task.end {
                            Err(cst_step_node_internal_error_spec())
                        } else {
                            cst_step_flow_sequence_finish_pair_spec(
                                machine,
                                positioned,
                                cst_single_pair_mapping_spec(
                                    with_empty,
        ...
        cst_step_flow_sequence_finish_entry_spec(machine, builder, task, task.key_node_index)
    }
}

pub open spec fn cst_step_flow_sequence_state_three_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::FlowSequence || task.state != 3 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
            ...
            Some((taken, child)) => {
                if child.next_token > task.end {
                    ...
                    let positioned = cst_task_set_cursor_spec(task, cursor as u64);
                    if task.entry_token_start > cursor {
                        Err(cst_step_node_internal_error_spec())
                    } else {
                        match cst_single_pair_mapping_spec(
                            builder,
                            tokens,
                            task.key_node_index,
                            child.node_index,
                            ...
                            Some(task.colon_token),
                        ) {
                            Err(error) => Err(error),
                            Ok((next_builder, pair)) => {
                                let entry = CstSequenceEntryView {
                                    node_index: pair,
                                    token_start: task.entry_token_start,
...
pub open spec fn cst_step_flow_mapping_state_zero_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
    ...
    if task.kind != ParseTaskKind::FlowMapping || task.state != 0 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        let cursor = cst_skip_trivia_spec(
        ...
        if cursor >= task.end {
            ...
            let kind = tokens[cursor].kind;
            if kind == CompletedTokenKind::FlowMappingEnd {
                cst_step_node_store_parsed_spec(
                    machine,
                    cst_finish_iterative_mapping_spec(
                        tokens,
...
pub open spec fn cst_step_flow_mapping_state_one_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::FlowMapping || task.state != 1 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
...
pub open spec fn cst_step_flow_mapping_finish_entry_spec(
    machine: ParseMachineView,
    builder: CstBuilderView,
    task: ParseTaskView,
    value_node_index: u64,
    ...
    let appended = cst_task_push_mapping_entry_spec(task, entry);
    let waiting = cst_task_set_state_spec(appended, 4);
    Ok((cst_machine_resume_task_spec(machine, waiting), builder))
}

pub open spec fn cst_step_flow_mapping_state_two_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
    ...
    if task.kind != ParseTaskKind::FlowMapping || task.state != 2 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else if task.cursor < task.end && tokens[task.cursor as int].kind
        == CompletedTokenKind::MappingValue {
        if task.entry_token_start >= task.end {
            ...
            let positioned = cst_task_set_cursor_spec(with_colon, cursor as u64);
            if cursor >= task.end || tokens[cursor].kind == CompletedTokenKind::FlowEntry
                || tokens[cursor].kind == CompletedTokenKind::FlowMappingEnd {
                match cst_empty_node_spec(builder, tokens, cursor as u64) {
...
pub open spec fn cst_step_flow_mapping_state_three_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::FlowMapping || task.state != 3 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
...
pub open spec fn cst_step_flow_mapping_state_four_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::FlowMapping || task.state != 4 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else if task.cursor >= task.end {
        Err(
...
pub open spec fn cst_step_block_sequence_state_one_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::BlockSequence || task.state != 1 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
            ...
            Some((taken, child)) => {
                if child.next_token > task.end || task.colon_token >= task.end {
                    ...
                    let ended = cst_task_set_entry_end_spec(positioned, child.next_token);
                    let extended = cst_task_extend_node_end_spec(ended, child.next_token);
                    let entry = CstSequenceEntryView {
                        node_index: child.node_index,
                        token_start: task.entry_token_start,
                        token_end: child.next_token,
                        indicator_token: Some(task.entry_token_start),
                    };
                    let appended = cst_task_push_sequence_entry_spec(extended, entry);
                    let waiting = cst_task_set_state_spec(appended, 2);
                    Ok((cst_machine_resume_task_spec(taken, waiting), builder))
                }
            },
...
pub open spec fn cst_step_block_mapping_state_one_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::BlockMapping || task.state != 1 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
            ...
            Some((taken, child)) => {
                if child.next_token > task.colon_token || task.colon_token > task.end {
                    ...
                    let remaining = cst_skip_trivia_spec(
                        tokens,
                        child.next_token as int,
                        task.colon_token as int,
                        (task.colon_token - child.next_token) as nat + 1,
                    );
                    if remaining != task.colon_token {
                        Err(
                            CstErrorView {
                                kind: CstErrorKind::UnexpectedToken,
                                byte_offset: tokens[remaining].byte_start,
                            },
                        )
                    } else {
                        let with_key = cst_task_set_key_node_spec(task, child.node_index);
...
pub open spec fn cst_step_block_mapping_state_four_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::BlockMapping || task.state != 4 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_machine_take_completed_spec(machine) {
            ...
            Some((taken, child)) => {
                if child.next_token > task.end {
                    ...
                    let positioned = cst_task_set_cursor_spec(with_key, cursor as u64);
                    let waiting = cst_task_set_state_spec(positioned, 5);
                    Ok((cst_machine_resume_task_spec(taken, waiting), builder))
                }
            },
        }
...
pub open spec fn cst_step_block_mapping_state_five_spec(
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    builder: CstBuilderView,
) -> Result<(ParseMachineView, CstBuilderView), CstErrorView> {
    if task.kind != ParseTaskKind::BlockMapping || task.state != 5 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else if task.entry_token_start >= task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        match cst_empty_node_spec(builder, tokens, task.cursor) {
            Err(error) => Err(error),
            Ok((next_builder, value)) => {
                let entry_token_end = if task.cursor > task.entry_token_start + 1 {
                    ...
                    (task.entry_token_start + 1) as u64
                };
                let ended = cst_task_set_entry_end_spec(task, entry_token_end);
                let entry = CstMappingEntryView {
                    key_node_index: task.key_node_index,
                    value_node_index: value,
                    token_start: task.entry_token_start,
                    token_end: entry_token_end,
                    explicit_key_token: Some(task.entry_token_start),
...
pub open spec fn cst_step_block_mapping_state_two_spec(
    atoms: Seq<crate::atom::LexicalAtomView>,
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    ...
    if task.kind != ParseTaskKind::BlockMapping || task.state != 2 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else if task.colon_token >= task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        let cursor = cst_skip_trivia_spec(
            tokens,
            task.colon_token as int + 1,
            task.end as int,
        ...
        if empty_value {
            match cst_empty_node_spec(builder, tokens, cursor as u64) {
                Err(error) => Err(error),
                Ok((next_builder, value)) => {
                    let entry_token_end = if cursor > task.colon_token as int + 1 {
                        cursor as u64
                    } else {
                        (task.colon_token + 1) as u64
                    };
            ...
            let boundary = cst_block_property_only_end_spec(
                ...
                task.end as int,
                task.indentation,
                (task.end as int - cursor) as nat + 1,
            );
            let child_end = match boundary {
                Some(index) => index as u64,
                None => task.end,
            };
            let allow_block_collection = cst_block_value_allows_collection_spec(
                tokens,
                cursor,
                child_end as int,
                task.colon_token as int,
                (child_end as int - cursor) as nat + 1,
            );
...
pub open spec fn cst_step_block_mapping_state_zero_spec(
    atoms: Seq<crate::atom::LexicalAtomView>,
    tokens: Seq<crate::token::CompletedTokenView>,
    task: ParseTaskView,
    machine: ParseMachineView,
    ...
    if task.kind != ParseTaskKind::BlockMapping || task.state != 0 || task.end > tokens.len()
        || task.cursor > task.end || task.token_start > task.end {
        Err(cst_step_node_internal_error_spec())
    } else {
        let cursor = cst_skip_trivia_spec(
        ...
        if cursor >= task.end || cst_token_column_spec(atoms, tokens[cursor]) != task.indentation {
            if task.pending_mapping.len() == 0 {
                ...
                cst_step_node_store_parsed_spec(
                    machine,
                    cst_finish_iterative_mapping_spec(tokens, positioned, None, builder),
                )
            }
        } else {
            ...
            let colon = if explicit {
                cst_find_explicit_mapping_value_spec(
                    atoms,
                    tokens,
                    keyed.cursor as int,
                    task.end as int,
                    task.indentation,
                    (task.end - keyed.cursor) as nat + 1,
                )
            } else {
                cst_find_mapping_value_on_line_spec(
                    tokens,
            ...
            match colon {
                None => {
                    if !explicit {
                        if task.pending_mapping.len() == 0 {
                            ...
                            cst_step_node_store_parsed_spec(
                                machine,
                                cst_finish_iterative_mapping_spec(tokens, keyed, None, builder),
                            )
                        }
                    } else {
                        ...
                        if empty_key {
                            match cst_empty_node_spec(builder, tokens, keyed.cursor) {
                                Err(error) => Err(error),
                                Ok((next_builder, key_node_index)) => {
                                    let with_key = cst_task_set_key_node_spec(
                                        keyed,
                                        key_node_index,
                                    );
                            ...
                            let waiting = cst_task_set_state_spec(keyed, 4);
                            let resumed = cst_machine_resume_task_spec(machine, waiting);
                            match cst_machine_push_task_spec(
                                resumed,
                                child,
                ...
                Some(colon_index) => {
                    let with_colon = cst_task_set_colon_spec(keyed, colon_index as u64);
                    if keyed.cursor >= colon_index {
                        match cst_empty_node_spec(builder, tokens, colon_index as u64) {
                            Err(error) => Err(error),
                            Ok((next_builder, key_node_index)) => {
                                let with_key = cst_task_set_key_node_spec(
                                    with_colon,
                        ...
                        let waiting = cst_task_set_state_spec(with_colon, 1);
                        let resumed = cst_machine_resume_task_spec(machine, waiting);
                        match cst_machine_push_task_spec(
                            resumed,
                            child,
...
#[verifier::rlimit(500)]
fn step_flow_sequence_state_zero(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_sequence_state_zero_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_sequence_state_zero_spec);
    }
    ...
    proof {
        assert(kind == token_views[task.cursor as int].kind);
        assert(positioned.cursor == task.cursor as u64);
    }
    if kind == CompletedTokenKind::FlowSequenceEnd {
    ...
    let explicit_key = kind == CompletedTokenKind::ExplicitMappingKey;
    let entry_start = task.cursor;
    task_begin_keyed_entry(entry_start, explicit_key, &mut task);
    let ghost started = task@;
    proof {
        assert(started == cst_task_begin_keyed_entry_spec(
            positioned,
            positioned.cursor,
            explicit_key,
        ));
        assert(started.explicit_key == explicit_key);
    }
    if task.explicit_key {
        let _prior_cursor = task.cursor;
        let cursor = skip_trivia(tokens, task.cursor + 1, task.end);
        task_set_cursor(cursor, &mut task);
        proof {
            assert(_prior_cursor as u64 == positioned.cursor);
            assert(cursor as int == cst_skip_trivia_spec(
                token_views,
                positioned.cursor as int + 1,
                task_view.end as int,
                (task_view.end as int - positioned.cursor as int) as nat,
            ));
            assert(task@ == cst_task_set_cursor_spec(started, cursor as u64));
        }
    }
    let ghost keyed = task@;
    proof {
        if explicit_key {
            assert(keyed == cst_task_set_cursor_spec(
                started,
                cst_skip_trivia_spec(
                    token_views,
                    positioned.cursor as int + 1,
            ...
            assert(keyed == started);
        }
        if keyed.cursor < task_view.end {
            crate::token::lemma_completed_token_view_at(tokens@, keyed.cursor as int);
    ...
    if task.cursor >= task.end || tokens[task.cursor].kind() == CompletedTokenKind::MappingValue
        ...
        task_set_key_node(node, &mut task);
        task_set_state(&mut task, 2);
        machine_resume_task(task, machine);
        proof {
            assert(cst_empty_node_spec(prior_builder, token_views, keyed.cursor) == Ok(
                (builder@, node),
            ));
            assert(keyed.cursor >= task_view.end || token_views[keyed.cursor as int].kind
                == CompletedTokenKind::MappingValue || token_views[keyed.cursor as int].kind
...
#[verifier::rlimit(500)]
fn step_flow_sequence_state_one(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_flow_sequence_state_one_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(_builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost expected = cst_step_flow_sequence_state_one_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_sequence_state_one_spec);
    }
    ...
    let cursor = skip_trivia(tokens, child.next_token, task.end);
    task_set_cursor(cursor, &mut task);
    task_set_state(&mut task, 2);
    machine_resume_task(task, machine);
    proof {
...
fn machine_finish_flow_sequence_entry(
    mut task: ParseTask,
    node_index: u64,
    machine: &mut ParseMachine,
)
    requires
        task@.kind == ParseTaskKind::FlowSequence,
    ensures
        final(machine)@ == cst_machine_resume_task_spec(
            old(machine)@,
            cst_task_set_state_spec(
                cst_task_push_sequence_entry_spec(
                    task@,
                    CstSequenceEntryView {
                        node_index,
                        token_start: task@.entry_token_start,
...
#[verifier::rlimit(700)]
fn step_flow_sequence_state_two(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_sequence_state_two_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_sequence_state_two_spec);
    }
    if task.cursor < task.end {
        proof {
            crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
    ...
    if task.cursor < task.end && tokens[task.cursor].kind() == CompletedTokenKind::MappingValue {
        ...
        proof {
            crate::token::lemma_completed_token_view_at(tokens@, task.entry_token_start as int);
        }
        ...
        let colon = task.cursor;
        task_set_colon(colon, &mut task);
        let cursor = skip_trivia(tokens, task.cursor + 1, task.end);
        task_set_cursor(cursor, &mut task);
        let ghost positioned = task@;
        if task.cursor >= task.end || tokens[task.cursor].kind() == CompletedTokenKind::FlowEntry
            || tokens[task.cursor].kind() == CompletedTokenKind::FlowSequenceEnd {
            if task.cursor < task.end {
                proof {
                    crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
                }
            }
            let ghost before_empty = builder@;
            ...
            let pair = match single_pair_mapping(
                ...
                Err(error) => {
                    proof {
                        assert(cst_single_pair_mapping_spec(
                            with_empty,
                            token_views,
                            positioned.key_node_index,
                            value,
                            positioned.entry_token_start,
                            pair_end as u64,
                            if positioned.explicit_key {
                                Some(positioned.entry_token_start)
                            } else {
                                None
                            },
                            Some(positioned.colon_token),
                        ) == Err(error@));
                        reveal(cst_step_flow_sequence_finish_pair_spec);
                        assert(expected == Err(error@));
                    }
                    return Err(error);
                },
            };
            machine_finish_flow_sequence_entry(task, pair, machine);
            proof {
                assert(cst_empty_node_spec(before_empty, token_views, positioned.cursor) == Ok(
                    (with_empty, value),
                ));
                assert(cst_single_pair_mapping_spec(
                    with_empty,
                ...
                reveal(cst_step_flow_sequence_finish_pair_spec);
                reveal(cst_step_flow_sequence_finish_entry_spec);
                assert(expected == Ok((machine@, builder@)));
            }
            return Ok(());
        }
    ...
    if task.explicit_key {
        ...
        let value = match empty_node(builder, tokens, task.cursor) {
            ...
            Err(error) => {
                proof {
                    assert(cst_empty_node_spec(before_empty, token_views, task_view.cursor) == Err(
                        error@,
                    ));
                    assert(expected == Err(error@));
                }
            ...
            Err(error) => {
                proof {
                    assert(cst_single_pair_mapping_spec(
                        with_empty,
                        token_views,
                        task_view.key_node_index,
                        value,
                        task_view.entry_token_start,
                        task_view.cursor,
                        Some(task_view.entry_token_start),
                        None,
                    ) == Err(error@));
                    reveal(cst_step_flow_sequence_finish_pair_spec);
                    assert(expected == Err(error@));
        ...
        machine_finish_flow_sequence_entry(task, pair, machine);
        proof {
            assert(cst_empty_node_spec(before_empty, token_views, task_view.cursor) == Ok(
                (with_empty, value),
            ));
            assert(cst_single_pair_mapping_spec(
                with_empty,
    ...
    let key_node = task.key_node_index;
    machine_finish_flow_sequence_entry(task, key_node, machine);
    proof {
        reveal(cst_step_flow_sequence_finish_entry_spec);
        assert(expected == Ok((machine@, builder@)));
    }
...
#[verifier::rlimit(500)]
fn step_flow_sequence_state_three(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_flow_sequence_state_three_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_sequence_state_three_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_sequence_state_three_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
        ...
        Err(error) => {
            proof {
                assert(cst_single_pair_mapping_spec(
                    ...
                    task_view.entry_token_start,
                    positioned.cursor,
                    if task_view.explicit_key {
                        Some(task_view.entry_token_start)
                    } else {
                        None
                    },
                    Some(task_view.colon_token),
                ) == Err(error@));
                assert(expected == Err(error@));
            }
            return Err(error);
...
#[verifier::rlimit(500)]
fn step_flow_mapping_state_zero(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
    ...
    requires
        task@.kind == ParseTaskKind::FlowMapping,
        task@.state == 0,
        task.end <= tokens.len(),
        task.cursor <= task.end,
        task.token_start <= task.end,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_mapping_state_zero_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_mapping_state_zero_spec);
    }
    let cursor = skip_trivia(tokens, task.cursor, task.end);
    task_set_cursor(cursor, &mut task);
    let ghost positioned = task@;
...
#[verifier::rlimit(500)]
fn step_flow_mapping_state_one(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    _builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_flow_mapping_state_one_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(_builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = _builder@;
    let ghost expected = cst_step_flow_mapping_state_one_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_mapping_state_one_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
...
fn machine_finish_flow_mapping_entry(
    mut task: ParseTask,
    value_node_index: u64,
    mapping_value_token: Option<u64>,
    machine: &mut ParseMachine,
    ...
    ensures
        final(machine)@ == cst_machine_resume_task_spec(
            old(machine)@,
            cst_task_set_state_spec(
                cst_task_push_mapping_entry_spec(
                    task@,
                    CstMappingEntryView {
                        key_node_index: task@.key_node_index,
                        value_node_index,
...
#[verifier::rlimit(600)]
fn step_flow_mapping_state_two(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_mapping_state_two_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_mapping_state_two_spec);
    }
    if task.cursor < task.end {
        proof {
            crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
    ...
    if task.cursor < task.end && tokens[task.cursor].kind() == CompletedTokenKind::MappingValue {
        ...
        let ghost positioned = task@;
        if task.cursor >= task.end || tokens[task.cursor].kind() == CompletedTokenKind::FlowEntry
            || tokens[task.cursor].kind() == CompletedTokenKind::FlowMappingEnd {
            if task.cursor < task.end {
                proof {
                    crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
                }
            ...
            machine_finish_flow_mapping_entry(task, value, Some(colon as u64), machine);
            proof {
                assert(cst_empty_node_spec(before_empty, token_views, positioned.cursor) == Ok(
                    (builder@, value),
                ));
                reveal(cst_step_flow_mapping_finish_entry_spec);
                assert(expected == Ok((machine@, builder@)));
            }
            return Ok(());
        }
    ...
    machine_finish_flow_mapping_entry(task, value, None, machine);
    proof {
        assert(cst_empty_node_spec(before_empty, token_views, task_view.cursor) == Ok(
            (builder@, value),
        ));
...
#[verifier::rlimit(500)]
fn step_flow_mapping_state_three(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    _builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_flow_mapping_state_three_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(_builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = _builder@;
    let ghost expected = cst_step_flow_mapping_state_three_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_mapping_state_three_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
    ...
    task_set_cursor(cursor, &mut task);
    let ghost positioned = task@;
    let colon = task.colon_token;
    machine_finish_flow_mapping_entry(task, child.node_index, Some(colon as u64), machine);
    proof {
        reveal(cst_step_flow_mapping_finish_entry_spec);
        assert(positioned.colon_token == task_view.colon_token);
        assert(expected == Ok((machine@, _builder@)));
    }
    Ok(())
}
...
#[verifier::rlimit(500)]
fn step_flow_mapping_state_four(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_flow_mapping_state_four_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_flow_mapping_state_four_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_flow_mapping_state_four_spec);
    }
    if task.cursor >= task.end {
        let error = CstError::at(CstErrorKind::UnexpectedEndOfInput, builder.source_len_bytes);
        proof {
...
#[verifier::rlimit(500)]
fn step_block_sequence_state_one(
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_block_sequence_state_one_spec(
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = _builder@;
    let ghost expected = cst_step_block_sequence_state_one_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        ...
        reveal(cst_step_block_sequence_state_one_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
...
fn step_block_mapping_state_zero(
    atoms: &[LexicalAtom],
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_block_mapping_state_zero_spec(
        atom_views,
        token_views,
        task_view,
        initial_machine,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_block_mapping_state_zero_spec);
    }
    let cursor = skip_trivia(tokens, task.cursor, task.end);
    task_set_cursor(cursor, &mut task);
    let ghost positioned = task@;
    ...
    proof {
        assert(task.cursor < task.end);
        crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
        assert(tokens@[task.cursor as int]@ == token_views[task.cursor as int]);
    }
    let explicit = tokens[task.cursor].kind() == CompletedTokenKind::ExplicitMappingKey;
    task_begin_keyed_entry(task.cursor, explicit, &mut task);
    let ghost started = task@;
    if task.explicit_key {
        let cursor = skip_trivia(tokens, task.cursor + 1, task.end);
        task_set_cursor(cursor, &mut task);
    ...
    if colon.is_none() {
        ...
        proof {
            assert(task.entry_token_start < task.end);
            crate::token::lemma_completed_token_view_at(tokens@, task.entry_token_start as int);
            assert(tokens@[task.entry_token_start as int]@
    ...
    let colon_index = colon.unwrap();
    proof {
        assert(task.cursor <= colon_index < task.end);
        crate::token::lemma_completed_token_view_at(tokens@, colon_index as int);
        assert(tokens@[colon_index as int]@ == token_views[colon_index as int]);
    }
    task_set_colon(colon_index, &mut task);
    let ghost with_colon = task@;
    ...
    proof {
        assert(task.cursor < task.colon_token);
        crate::token::lemma_completed_token_view_at(tokens@, task.cursor as int);
        assert(tokens@[task.cursor as int]@ == token_views[task.cursor as int]);
    }
    let child_offset = tokens[task.cursor].byte_start();
...
#[verifier::rlimit(500)]
fn step_block_mapping_state_one(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    _builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_block_mapping_state_one_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(_builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = _builder@;
    let ghost expected = cst_step_block_mapping_state_one_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_block_mapping_state_one_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
    ...
    if remaining != task.colon_token {
        proof {
            crate::token::lemma_completed_token_view_at(tokens@, remaining as int);
            assert(tokens@[remaining as int]@ == token_views[remaining as int]);
        }
        let error = CstError::at(CstErrorKind::UnexpectedToken, tokens[remaining].byte_start());
        proof {
            assert(expected == Err(error@));
        }
        return Err(error);
...
#[verifier::rlimit(500)]
fn step_block_mapping_state_four(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    _builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_block_mapping_state_four_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(_builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = _builder@;
    let ghost expected = cst_step_block_mapping_state_four_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_block_mapping_state_four_spec);
    }
    let child = match machine_take_completed(machine) {
        Some(parsed) => parsed,
        None => {
...
#[verifier::rlimit(500)]
fn step_block_mapping_state_five(
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
    builder: &mut CstBuilder,
    ...
    requires
        ...
        task.token_start <= task.end,
    ensures
        cst_step_block_mapping_state_five_spec(
            crate::token::completed_token_views_spec(tokens@),
            task@,
            old(machine)@,
            old(builder)@,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_block_mapping_state_five_spec(
        token_views,
        task_view,
        initial_machine,
        initial_builder,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_block_mapping_state_five_spec);
    }
    if task.entry_token_start >= task.end {
        proof {
            reveal(cst_step_node_internal_error_spec);
    ...
    let value = match empty_node(builder, tokens, task.cursor) {
        ...
        Err(error) => {
            proof {
                assert(cst_empty_node_spec(prior_builder, token_views, task_view.cursor) == Err(
                    error@,
                ));
                assert(expected == Err(error@));
            }
    ...
    machine_resume_task(task, machine);
    proof {
        assert(cst_empty_node_spec(prior_builder, token_views, task_view.cursor) == Ok(
            (builder@, value),
        ));
        assert(expected == Ok((machine@, builder@)));
    }
...
#[verifier::rlimit(800)]
fn step_block_mapping_state_two(
    atoms: &[LexicalAtom],
    tokens: &[CompletedToken],
    mut task: ParseTask,
    machine: &mut ParseMachine,
        ...
        ) && result.is_ok() ==> cst_parse_task_stack_is_valid_spec(final(machine)@.tasks),
{
    ...
    let ghost initial_machine = machine@;
    let ghost initial_builder = builder@;
    let ghost expected = cst_step_block_mapping_state_two_spec(
        atom_views,
        token_views,
        task_view,
        initial_machine,
    ...
    proof {
        crate::token::lemma_completed_token_views_len(tokens@);
        reveal(cst_step_block_mapping_state_two_spec);
    }
    if task.colon_token >= task.end {
        proof {
            reveal(cst_step_node_internal_error_spec);
    ...
    proof {
        crate::token::lemma_completed_token_view_at(tokens@, task.colon_token as int);
        assert(tokens@[task.colon_token as int]@ == token_views[task.colon_token as int]);
    }
    let cursor = skip_trivia(tokens, task.colon_token + 1, task.end);
    task_set_cursor(cursor, &mut task);
    let ghost positioned = task@;
    if task.cursor < task.end {
        proof {
    ...
    let cursor_column = if task.cursor < task.end {
        token_column(atoms, &tokens[task.cursor])
    } else {
        0
    };
    ...
    if empty_value {
        ...
        machine_resume_task(task, machine);
        proof {
            assert(cst_empty_node_spec(prior_builder, token_views, positioned.cursor) == Ok(
                (builder@, value),
            ));
            assert(expected == Ok((machine@, builder@)));
        }
    ...
    let child = node_task(task.cursor, child_end, allow_block_collection, task.depth_left);
    task_set_state(&mut task, 3);
    machine_resume_task(task, machine);
    let ghost resumed = machine@;
    match machine_push_task(child, depth_limit, child_offset, machine) {
