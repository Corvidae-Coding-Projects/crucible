...
| Crucible YAML implementation | Verified profile-1 UTF-8/BOM decoder, bounded lexical atoms, exact line/indentation layout, lossless structural candidates, authenticated context-sensitive quoted/plain/block scalars, and completed YAML token formation. The completed-token scanner authenticates every upstream witness, retains trivia and exact directive/property subranges, enforces typed contextual indicators and bounded flow nesting, forms a lossless atom/byte partition, and has total executable-to-pure success/error correspondence plus public semantic, partition, and flow-balance proofs. Parser/CST formation, resolution, lowering, schema validation, canonical serialization, and self-fuzzing remain committed work |
