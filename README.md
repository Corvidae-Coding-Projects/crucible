...
| Crucible YAML implementation | Verified profile-1 UTF-8/BOM decoder, bounded lexical atoms, exact line/indentation layout, lossless structural candidates, and authenticated context-sensitive quoted- and plain-scalar boundaries. The scanner preserves raw presentation, validates YAML-printable content and complete quoted escapes, excludes node properties and block regions, decides leading-tab legality contextually, and proves exact/non-overlapping ranges, caps, and total error correspondence. Completed block-scalar and final-token formation, parser, resolution, lowering, schema validation, canonical serialization, and self-fuzzing remain committed work |
