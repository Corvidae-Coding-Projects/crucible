...
| Crucible YAML implementation | Verified profile-1 UTF-8/BOM decoder, bounded lexical atoms, exact line/indentation layout, lossless structural candidates, and authenticated context-sensitive quoted, plain, literal-block, and folded-block scalar formation. Block scalars implement both header-modifier orders, compact-collection-aware automatic and explicit indentation, contextual tabs, strip/clip/keep chomping, YAML folding around empty and more-indented lines, four exact caps, and code-point source provenance with generally proved exact rendered content and non-overlapping ranges. Final-token formation, parser, resolution, lowering, schema validation, canonical serialization, and self-fuzzing remain committed work |
